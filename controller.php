...
// Lisab uue kirje
function controller_add($probleem)
{
    if (!controller_user()) {
...
function controller_filter($filter)
{
    if (!controller_user()) {
        message_add('Tegevus eeldab sisselogimist');
    ...
    //filter puudu või sisend vale
    if (!$filter || !in_array($filter, array('all','solved','not_solved'))) {
        return false;
    }
